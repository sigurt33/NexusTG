...
- **Фрилансерам и консультантам** — много чатов с клиентами, важно не пропустить ЛС и упоминания.
- **Тимлидам и менеджерам** — десятки рабочих групп, нужны только адресные сообщения, а не флуд.
- **Активным участникам сообществ** — отслеживать ответы и @mentions в больших чатах.
- **Тем, кто страдает от перегрузки Telegram** — заменяет ленту чистым inbox'ом с приоритетами.
- **Любителям приватности** — всё локально, без облаков; данные и сессия не уходят наружу.
- **Аналитикам собственной переписки** — отчёты по темам/чатам/отправителям, CSV-экспорт.
- **Разработчикам как база** — расширяемый каркас (Telethon + FastAPI + LLM) для своих инструментов поверх Telegram.
...
- **Транскрипция голосовых + обработка медиа.** Голосовые до N минут (`voice_transcribe_max_minutes`) автоматически расшифровываются через Gemini и дальше классифицируются как обычный текст. Длинные голосовые, фото, видео и документы получают максимальный приоритет с пометкой «открой в Telegram»; стикеры/GIF — тривиальные.
...
# Голосовые до этого порога (мин) транскрибируются через Gemini; длиннее — метка + приоритет 5
...
В папке [`skills/`](skills/) лежат два скилла, помогающие AI-агенту (Claude Code) работать с проектом:

- **[`nexustg-dev`](skills/nexustg-dev/SKILL.md)** — карта проекта, запуск сервиса, рабочий процесс по внесению изменений, проверка ошибок (логи, compile-чек), правила обновления README и чек-лист подготовки к публикации (gitignore, утечки секретов, блюр скринов).
- **[`nexustg-lite`](skills/nexustg-lite/SKILL.md)** — описание «лёгкого» режима NexusTG: только сбор ЛС, упоминаний и ответов в SQLite. Без веба, без LLM-классификатора, без бота, без контекста и backfill. Минимальный `config.toml`, минимальный `.env`, инструкции по флагу `--lite`.

Скиллы автоматически подхватываются Claude Code при работе в этом репозитории.

---

## Статус

Phase 5+: ingestion + LLM-классификация + веб + бот + отчёты + CSV-экспорт + обучающие примеры + задачник с напоминаниями о дедлайнах + вкладка «Настройки» + транскрипция голосовых и обработка медиа.
